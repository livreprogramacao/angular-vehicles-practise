...
export class TablePagination<T> {
    ...
    fetchNextPage() {
        ...
        let observable: Rx.Observable<any> = this.table.fetchPage(this.page.number, this.page.size, this.getSort());
        if (observable != null) {
            showLoading();
            observable.subscribe(
                () => { },
...
export class TableSort<T> implements OnChanges {
    ...
    ngOnChanges(changes: any) {
        ...
        if (changes['page']) {
            ...
            var defineValues = (s: boolean, sa: boolean, sd: boolean, dir: string) => {
                this.sortClass = s;
